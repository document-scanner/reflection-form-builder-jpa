...
public class QueryPanel<E> extends AbstractQueryPanel<E> {
     ...
     * @param entityClassFields the entity class fields
     * @return the set of fields as specified
     */
        ...
        for(Field entityClassField : entityClassFields) {
            for(Field entityClassFieldField : entityClassField.getType().getDeclaredFields()) {
                ...
                if((entityClassFieldOneToOne != null || entityClassFieldManyToOne != null)
                        && entityClass.isAssignableFrom(entityClassFieldField.getType())) {
    ...
    */
     ...
     * @param queryResultTableHeight the query result table height
     * @param bidirectionalControlPanel the bidirection control panel to use
     * @throws FieldHandlingException if such an exception occurs
     * @throws ResetException if the initial reset fails
     * @throws IllegalArgumentException if {@code initialSelectedHistoryEntry}
     *     is not {@code null}, but not contained in {@code initialHistory}
     */
    public QueryPanel(PersistenceStorage storage,
            Class<E> entityClass,
            IssueHandler issueHandler,
